...
<div class="container mt-4">
    ...
    <?php endif; ?>
    <form action="<?= base_url('guru/materitugas/tugasStore') ?>" method="post" enctype="multipart/form-data" class="mt-3">
        <?= csrf_field() ?>
        <div class="mb-3">
            ...
            <select name="kd_kelas" id="kd_kelas" class="form-select" required>
                ...
                <?php if (!empty($kelas_diampu)): foreach ($kelas_diampu as $k): ?>
                        <option value="<?= $k['kd_kelas'] ?>" <?= old('kd_kelas') == $k['kd_kelas'] ? 'selected' : '' ?>><?= esc($k['nama_kelas']) ?></option>
                <?php endforeach;
                endif; ?>
            </select>
        </div>
        <div class="mb-3">
            ...
            <select name="kd_mapel" id="kd_mapel" class="form-select" required>
                ...
                <?php if (!empty($mapel_diampu)): foreach ($mapel_diampu as $m): ?>
                        <option value="<?= $m['kd_mapel'] ?>" <?= old('kd_mapel') == $m['kd_mapel'] ? 'selected' : '' ?>><?= esc($m['nama_mapel']) ?></option>
                <?php endforeach;
                endif; ?>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <small class="form-text text-muted">Upload file tugas yang akan diberikan kepada siswa (opsional)</small>
        </div>
        <button type="submit" class="btn btn-primary"><i class="fas fa-plus"></i> Simpan Tugas</button>

...
class Nilai extends BaseController
{
    ...
    public function index()
    {
        ...
        $nilai = $this->nilaiModel->select('nilai.*, mapel.nama_mapel')
            ...
            ->where('nilai.siswa_id', $siswa['nis'])
        ...
        log_message('debug', 'ISI RANKING: ' . json_encode($ranking));
        log_message('debug', 'DEBUG STRUCTURE RANKING: ' . print_r($ranking, true));
        $peringkat = 0;
        if (!empty($ranking)) {
            foreach ($ranking as $i => $r) {
                log_message('debug', 'DEBUG RANKING ITEM: ' . print_r($r, true));
                if (isset($r['id'])) {
                    log_message('error', 'RANKING ITEM HAS UNEXPECTED KEY id: ' . print_r($r, true));
                }
            ...
            log_message('error', 'Ranking array is empty or not valid.');
        }
    ...
    public function detail($mapel_id)
    {
        ...
        $rataRataKelas = $this->nilaiModel->select('AVG(akhir) as rata_rata')
            ->join('siswa', 'siswa.nis = nilai.siswa_id')
            ->where('siswa.kd_kelas', $siswa['kd_kelas'])
            ->where('nilai.kd_mapel', $mapel_id)
            ->where('nilai.semester', $semester)
            ->where('nilai.tahun_akademik_id', $tahunAkademikAktif['id'])
            ->where('nilai.akhir >', 0)
    ...
    public function raport()
    {
        ...
        // Ambil data absensi
        $absensi = $this->absensiModel->where('siswa_id', $siswa['nis'])->findAll();
